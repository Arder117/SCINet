...
class SRModel(BaseModel, nn.Module):  # 继承 torch.nn.Module
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):
        super(SRModel, self).__init__(opt)
        nn.Module.__init__(self)  # 确保初始化 nn.Module

        # define network
        self.net_g = build_network(opt['network_g'])
        self.net_g = self.model_to_device(self.net_g)  # 将模型转移到指定设备
        ...
        if load_path is not None:
            ...
            f = self.opt['path'].get('strict_load_g', True)
