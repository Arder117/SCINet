...
class Conv2d_Hori_Veri_Cross(nn.Module):
    ...
    def forward(self, x):
        # Get the dimensions of the conv weight
        [C_out, C_in, H_k, W_k] = self.conv.weight.shape
        tensor_zeros = torch.FloatTensor(C_out, C_in, 1).fill_(0).to(x.device)  # Ensure tensor_zeros is on the same device as x

        # Create new convolution weight by concatenating zeros
        ...
        conv_weight = conv_weight.contiguous().view(C_out, C_in, 3, 3)  # Reshape to [C_out, C_in, 3, 3]
        ...
        if math.fabs(self.theta - 0.0) < 1e-8:
            ...
            kernel_diff = kernel_diff[:, :, None, None]  # Add extra dimensions to match the conv2d input shape
...
class SCINet(nn.Module):
    ...
    #         for i in range(64):
    #             huatu = out_RB[:, i, :, :].squeeze().detach().cpu().numpy()  # 使用detach()防止反向传播
    #             huatu = huatu[:128, :160]
    #             huatu = cv2.resize(huatu, (640, 512))
    #             # Ensure that huatu has 3 channels
    #             huatu = np.stack([huatu] * 3, axis=-1)  # Convert to (H, W, 3)
    
    #             # Ensure the directory exists before saving
    #             save_dir = '/data/dl/SCINet-main/duibitu_qian/'
    #             if not os.path.exists(save_dir):
    #                 os.makedirs(save_dir)
    
    #             # Save the image
    #             plt.imsave(f'{save_dir}{i}.bmp', huatu, cmap='jet')
    ...
    #         for i in range(64):
    #             huatu2 = out_GB[:, i, :, :].squeeze().detach().cpu().numpy()  # 使用detach()防止反向传播
    #             huatu2 = huatu2[:128, :160]
    #             huatu2 = cv2.resize(huatu2, (640, 512))
    #             # Ensure that huatu2 has 3 channels
    #             huatu2 = np.stack([huatu2] * 3, axis=-1)  # Convert to (H, W, 3)
    
    #             # Ensure the directory exists before saving
    #             save_dir = '/data/dl/SCINet-main/duibitu_qian2/'
    #             if not os.path.exists(save_dir):
    #                 os.makedirs(save_dir)
    
    #             # Save the image
    #             plt.imsave(f'{save_dir}{i}.bmp', huatu2, cmap='jet')
    
    #     "[4,128,128,160]"
    #     x_f_cat = torch.cat([out_RB, out_GB], dim=1)
    #     if x_f_cat.shape[1] == 128:
    #         for i in range(128):
    #             huatu3 = x_f_cat[:, i, :, :].squeeze().detach().cpu().numpy()  # 使用detach()防止反向传播
    #             huatu3 = huatu3[:128, :160]
    #             huatu3 = cv2.resize(huatu3, (640, 512))
    #             # Ensure that huatu3 has 3 channels
    #             huatu3 = np.stack([huatu3] * 3, axis=-1)  # Convert to (H, W, 3)
    
    #             # Ensure the directory exists before saving
    #             save_dir = '/data/dl/SCINet-main/duibitu_qian3/'
    #             if not os.path.exists(save_dir):
    #                 os.makedirs(save_dir)
    
    #             # Save the image
    #             plt.imsave(f'{save_dir}{i}.bmp', huatu3, cmap='jet')
    ...
    #     if x_out.shape[1] == 64:
    #         for i in range(64):
    #             huatu4 = out_GB[:, i, :, :].squeeze().detach().cpu().numpy()  # 使用detach()防止反向传播
    #             huatu4 = huatu4[:128, :160]
    #             huatu4 = cv2.resize(huatu4, (640, 512))
    #             # Ensure that huatu4 has 3 channels
    #             huatu4 = np.stack([huatu4] * 3, axis=-1)  # Convert to (H, W, 3)
    
    #             # Ensure the directory exists before saving
    #             save_dir = '/data/dl/SCINet-main/duibitu_qian4/'
    #             if not os.path.exists(save_dir):
    #                 os.makedirs(save_dir)
    
    #             # Save the image
    #             plt.imsave(f'{save_dir}{i}.bmp', huatu4, cmap='jet')
    ...
    def forward(self, input):
        ...
        # --- 只有在非训练模式下才执行保存逻辑，且加入归一化 ---
        if not self.training:
            if out_RB.shape[1] == 64:
                save_dir = '/data/dl/SCINet-main/duibitu_qian/'
                os.makedirs(save_dir, exist_ok=True)
                for i in range(64):
                    huatu = out_RB[0, i, :, :].detach().cpu().numpy() # 取batch中的第一张
                    # 归一化到 0-1 防止 ValueError
                    huatu = (huatu - huatu.min()) / (huatu.max() - huatu.min() + 1e-8)
                    huatu = cv2.resize(huatu, (640, 512))
                    plt.imsave(f'{save_dir}{i}.bmp', huatu, cmap='jet')
        ...
        if not self.training:
            if out_GB.shape[1] == 64:
                ...
                os.makedirs(save_dir, exist_ok=True)
                for i in range(64):
                    huatu2 = out_GB[0, i, :, :].detach().cpu().numpy()
                    huatu2 = (huatu2 - huatu2.min()) / (huatu2.max() - huatu2.min() + 1e-8)
                    huatu2 = cv2.resize(huatu2, (640, 512))
                    plt.imsave(f'{save_dir}{i}.bmp', huatu2, cmap='jet')
        ...
        if not self.training:
            if x_f_cat.shape[1] == 128:
                ...
                os.makedirs(save_dir, exist_ok=True)
                for i in range(128):
                    huatu3 = x_f_cat[0, i, :, :].detach().cpu().numpy()
                    huatu3 = (huatu3 - huatu3.min()) / (huatu3.max() - huatu3.min() + 1e-8)
                    huatu3 = cv2.resize(huatu3, (640, 512))
                    plt.imsave(f'{save_dir}{i}.bmp', huatu3, cmap='jet')
        ...
        if not self.training:
            if x_out.shape[1] == 64:
                ...
                os.makedirs(save_dir, exist_ok=True)
                for i in range(64):
                    huatu4 = x_out[0, i, :, :].detach().cpu().numpy()
                    huatu4 = (huatu4 - huatu4.min()) / (huatu4.max() - huatu4.min() + 1e-8)
                    huatu4 = cv2.resize(huatu4, (640, 512))
                    plt.imsave(f'{save_dir}{i}.bmp', huatu4, cmap='jet')
